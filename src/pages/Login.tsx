import { IonAlert, IonButton, IonIcon, IonPage, IonSpinner, useIonRouter } from '@ionic/react'
...
const Login: React.FC<RouteComponentProps> = () => {
    ...
    const { form: { register, formState: { errors }, handleSubmit } } = useCustomForm(loginSchema);
    ...
    const submit = async (e: loginSchemaType) => {
        ...
        try {
            ...
            if (status === 200) {
                await Preferences.set({key: 'token', value: result.JWT})
    ...
    return (
        <IonPage>
            ...
            <div className='w-screen h-screen flex justify-center items-center relative oveeflow-hidden'>
                <GradientBackground />
                <div className='flex px-8 py-6 justify-start items-center flex-col gap-10 z-10 text-black dark:text-white'>
                    <div className='flex justify-start items-start flex-col gap-4'>
                        <span className='text-5xl font-extrabold'>
                            Welcome !!
                        </span>
                        <span className='text-xs font-light pr-8'>
                            Masukkan username dan password untuk masuk ke aplikasi pembayaran spp
                            <span className='font-bold ml-1'>SMK Cendekia Sungai Karang</span>
                        </span>
                    </div>
                    <form onSubmit={handleSubmit(submit)} className='w-full flex flex-col items-center gap-6'>
                        <div className='w-full flex justify-center items-start flex-col gap-4'>
                            ...
                            <div className='w-full relative'>
                                ...
                                <IonIcon
                                    ...
                                    className='absolute top-[14px] right-4 z-10' size='large'
                                />
                            </div>
                        </div>
